...
float BaseMQ::readRs(float temperature, float humidity) const {
  float rs = 0;
  ...
  rs = rs/(1.6979-0.012*temperature - 0.00612*humidity);
  return rs;
}

float BaseMQ::readRsJust() const {
  float rs = 0;
  for (int i = 0; i < MQ_SAMPLE_TIMES; i++) {
    rs += calculateResistance(analogRead(_pin));
    delay(MQ_SAMPLE_INTERVAL);
...
float BaseMQ::readScaled(float a, float b, float temperature, float humidity) const {
  float ratio = readRatio(temperature, humidity);
  return exp((log(ratio) - b) / a);
}

float BaseMQ::readRatio(float temperature, float humidity) const {
  return readRs(temperature, humidity) / getRo();
}
