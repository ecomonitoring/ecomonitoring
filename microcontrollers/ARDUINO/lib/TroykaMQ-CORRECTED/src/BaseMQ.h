...
public:
    ...
    float readRatio(float temperature, float humidity) const;
    inline bool isCalibrated() const { return _stateCalibrate; };
    inline float getRo() const { return _ro; };

protected:
    float readScaled(float a, float b, float temperature, float humidity) const;
...
private:
    ...
    uint8_t _pin;
    uint8_t _pinHeater;
    float readRs(float temperature, float humidity) const;
    float readRsJust() const;
